...
parser.add_argument('--lambda2', type=float, default=15.0)  # sparsity cost on C
...
parser.add_argument('--enable-at', type=int, default=185)  # epoch at which to enable eqn3plus
parser.add_argument('--dataset', type=str, default='coil20', choices=['yaleb', 'orl', 'coil20', 'coil100'])
...
CUDA_VISIBLE_DEVICES=0 python main.py orl_run1   --preimport tensorflow as tf
    pretrain for 10000 iterations first, then train on eqn3 for 2000 epochs, and on eqn3plus for 2000 epochs
    
Note: it is important to tune the parameters, especially for the postprocessing hyparameters.
"""
...
class ConvAE(object):
    def __init__(self,
        ...
        z = tf.reshape(latent, [int(batch_size), -1])
        ...
        # Eqn 3 loss ： encoder 与 decoder转换的损失，自表示系数范数，自表示损失，AE regularization loss
        ...
        Pretraining
        """
        # pretraining loss   encoder 与 decoder之间的损失（不含自表示层）
    ...
    def get_u_init_for_g(self, g):
        ...
        q, r = tf.qr(gt, full_matrices=True)
    ...
    def svd_initialization(self, z, y):
        group_index = [tf.where(tf.equal(y, k)) for k in range(self.n_class)]  # indices of datapoints in k-th cluster
        ...
        for i, g in enumerate(groups):
            ...
            if args.degerate:
                ...
                num_fake = tf.to_int32(tf.minimum(
                     tf.maximum(tf.to_int32(args.s_lambda2 * args.s_tau2 ** (self.gen_step)), tf.to_int32(0.4 * tf.to_float(N_g))), N_g))
                #print('================================================')
                #print(num_fake)
                #print('================================================')
...
def reinit_and_optimize(args, Img, Label, CAE, n_class, k=10, post_alpha=3.5, alpha = 0.1):
    ...
    for epoch in range(1, num_epochs + 1):
        # eqn3   在低于enable_at时，训练生成器，等于enable_at时初始化鉴别器
        ...
        if epoch % interval == 0:
            ...
            if (len(set(list(np.squeeze(y_x_new)))) == n_class) or (epoch == 10):   #已经修改
                ...
                print('================================================')
            # print('Cofe: {}  y_x:{}'.format(Coef, y_x))
            ###此处存在bug，若第一次聚类不足二十，则y_x未初始化,已修改
...
def prepare_data_orl(folder):
    mat = sio.loadmat(os.path.join(folder, '../../Deep Adversarial Subspace Clustering__zhoupan/dsc_code/ORL.mat'))
...
def prepare_data_coil20(folder):
    mat = sio.loadmat(os.path.join(folder, '../../Deep Adversarial Subspace Clustering__zhoupan/dsc_code/COIL20.mat'))
...
if __name__ == '__main__':
    ...
    logs_path = os.path.join(folder, '../../Deep Adversarial Subspace Clustering__zhoupan/dsc_code/logs', args.name)
    ...
    for n_class in all_subjects:
        ...
        CAE = ConvAE(
            ...
            reg=tf.keras.regularizers.l2(args.lambda4), disc_bound = args.bound,
